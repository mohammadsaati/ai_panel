...
class Menu
{
    ...
    {
        $permission = $this->getSubMenuPermissions(item : $item);
        if (!$permission)
        {
            echo '';
        }else {
    ...
    {
        if (!Auth::guard('admins')->check() || empty($permission))
            return true;
    ...
    private function getSubMenuPermissions($item)
    {
        $permissions = [];
        ...
        return $this->checkAdminPermission($permissions);
    }
    ...
    private function showSubItems($sub_items)
    {
        ...
        {
            $permission = $this->checkAdminPermission($sub_item['permission'] ?? '');
            if ($permission)
            {
